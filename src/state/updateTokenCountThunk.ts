...
export const updateTokenCountThunk = 
  createAsyncThunk<void, { conversationId: string }, { state: RootState }>(
    ...
    async ({ conversationId }, { getState, dispatch }) => {
      ...
      try {
        ...
        dispatch(updateTokenCount({ conversationId, totalTokens: totalTokens ?? 0, cachedContentTokenCount: cachedContentTokenCount ?? 0 }));
      } catch (error) {
